...
def main1():
    df = pd.read_csv('mult_siglipv2-g_train_mod_big_mapping.csv')
    result_df = compute_knn_predictions(df, k_values)
    result_df.to_csv('train_big_mapping.csv', index=False)


def main2():
    result_df = pd.read_csv('train_big_mapping.csv')
    print(len(result_df))
    accuracy_df = compute_knn_accuracies(result_df, k_values)
    accuracy_df.to_csv('train_big_mapping_acc.csv', index=False)
