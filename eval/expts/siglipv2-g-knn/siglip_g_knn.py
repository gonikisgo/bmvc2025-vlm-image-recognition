...
def compute_knn_predictions(df: pd.DataFrame, k_values: List[int]) -> pd.DataFrame:
    top_label_cols = [f'top_{i}_pred' for i in range(1, 52)]
    ...
    for k in k_values:
        ...
        print(f"k={k}, example row vote: {df[current_cols].iloc[0].values.tolist()}, majority: {result_df[f'k_{k}_pred'].iloc[0]}")
    return result_df
...
def compute_knn_accuracies(df: pd.DataFrame, k_values: list) -> pd.DataFrame:
    ...
    for k in k_values:
        ...
        accuracies.append({'k': k, 'accuracy': round(accuracy, 2)})
...
def main1():
    df = pd.read_csv('siglip2-g_nn_val.csv')
    print(len(df))
    result_df = compute_knn_predictions(df, k_values)
    result_df.to_csv('val_siglipv2-g_knn.csv', index=False)


def main2():
    result_df = pd.read_csv('val_siglipv2-g_knn.csv')
    print(len(result_df))
    accuracy_df = compute_knn_accuracies(result_df, k_values)
    accuracy_df.to_csv('val_siglipv2-g_knn_acc.csv', index=False)
